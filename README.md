...
- `specs/schedule-primary-location.md` defines the implemented additive contract for creating, referencing, replacing, clearing, and explicitly reading one primary location through the schedule surfaces while retaining the granular canonical location model; structural and behavioral coverage lives in `tests/test_schedule_primary_location_contract_fixtures.py` and `tests/test_schedule_primary_location_command.py`.
...
- `specs/notification-rendering.md` defines the implemented deterministic concise `en-CA` rendering profile for natural ordinary-reminder prose, immutable per-attempt rendering evidence, exact adapter-envelope binding, and `schedule.show` readback on schema 9.
